...
const ProjectsComponent = () => {
  ...
  return (
   ...
   {CreateComponent && <div className="createProject" style={{ height: '500px', width: '60%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'absolute', top: 90 }}>
     ...
     <input onChange={(e) => { setProjectData({ ...projectData, projectType: e.target.value }) }} className="input_field" type="text" placeholder="project Type" />
     <input onChange={(e) => { setProjectData({ ...projectData, skills: e.target.value }) }} className="input_field" type="text" placeholder="skills- ex:(Web/Mobile App)" />
